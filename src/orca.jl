...
function _read_orca_modes(dirname, zs, ϕ)
  ...
  map(3:length(s)) do i
    ...
    m = parse(Int, flds[1])
    kre = _parse_fortran_float(Float64, flds[2])
    att = _parse_fortran_float(Float64, flds[3])
    kᵣ = ComplexF64(kre * Kw, log(10^(-att/1000/20)))
    vₚ = _parse_fortran_float(Float64, flds[4])
    v = _parse_fortran_float(Float64, flds[5])
